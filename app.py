...
st.subheader("Team")
st.write("- **Никита**: EDA, Hypothesis check, streamlit")
st.write("- **Михаил**: EDA, streamlit")
st.write("- **Филипп**: EDA, Data cleanup")
...
st.write("Датасет содержит информацию о ведущих языковых моделях: их характеристики, результаты бенчмарков (MMLU, HumanEval, GPQA) и стоимость токенов.")

st.info('''
**Методология предобработки (Data Imputation):**
* В признаке `gpqa_diamond_score` было восстановлено 5 пропусков. Использован метод заполнения через корреляцию: MMLU модели умножался на среднее отношение GPQA к MMLU по всему датасету.
* Пропуски в `parameter_count_B` (20 моделей) были заполнены нулями, так как точное количество параметров у закрытых проприетарных моделей неизвестно.
''')
...
st.caption("сравнение моделей под узкие задачи")
...
st.caption("три главных измерения (размер железа, стоимость токенов и базовый ум) в одном пространстве для поиска кластеров и аномалий")
...
st.write("В ходе работы датасет был очищен от пропусков с использованием импутации метрики GPQA на основе корреляции с MMLU. Это позволило сохранить 12.5% выборки. Были сгенерированы новые признаки (год релиза, value_for_money). Проведенный визуальный и статистический анализ доказал, что открытые технологии полностью сравнялись по базовому качеству с коммерческими гигантами, а наценка за бренд не влияет на субъективное восприятие качества ответов реальными пользователями.")
with st.sidebar:
